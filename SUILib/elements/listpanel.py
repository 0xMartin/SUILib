...
class ListPanel(GUIElement, Container):
    ...
    def __init__(self, view, style: dict, data: list, width: int = 0, height: int = 0, x: int = 0, y: int = 0):
        ...
        self._hovered_row = None  # <-- Přidáno: aktuálně hovernutý index řádku
        ...
        self._v_scroll.add_event_callback(SUIEvents.EVENT_ON_CHANGE, self.scroll_vertical)
        self.refresh_list()
    ...
    def scroll_vertical(self, position: float):
        ...
        """
        total_body_data_height = 10 + (self.font.get_height() + 10) * len(self._data)
        h = super().get_height()
    ...
    def refresh_list(self, new_data: list = None):
        ...
        self.height = 10 + (self.font.get_height() + 10) * min(5, len(self._data))

        if self._v_scroll is not None:
            sw = super().get_style()["scrollbar_width"]
            self._v_scroll.set_x(super().get_x() + super().get_width() - sw)
            self._v_scroll.set_y(super().get_y())
            self._v_scroll.set_width(sw)
            self._v_scroll.set_height(super().get_height())

            height = 10 + (self.font.get_height() + 10) * len(self._data)
            self._v_scroll.set_scroller_size(
                (1.0 - max(0, height - super().get_height()) / height) * self._v_scroll.get_height()
            )
    ...
    def draw(self, view, screen):
        ...
        if len(self._data) != 0:
            ...
            offset = super().get_y() + 10 + self._body_offset_y
            for idx, line in enumerate(self._data):
                item_rect = pygame.Rect(
                    super().get_x(),
                    offset - 5,
                    super().get_width() - self._v_scroll.get_width() - 5,
                    self.font.get_height() + 10
                )
                # Zvýraznění pouze hovernutého řádku
                if idx == self._hovered_row:
