...
class VerticalScrollbar(GUIElement):
    ...
    def set_scroller_size(self, size: int):
        ...
        """
        self._scroller_size = max(size, super().get_width())
    ...
    def draw(self, view, screen):
        ...
        pygame.draw.rect(
            ...
            pygame.Rect(
                super().get_x(),
                super().get_y() + self._scroller_pos,
                super().get_width(),
                self._scroller_size
            ),
            border_radius=6
    ...
    def process_event(self, view, event):
        ...
        if self._scroller_size >= super().get_height():
            return
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            self._scroller_pos = self._start_scroller_pos + (event.pos[1] - self._drag_start)
            self._scroller_pos = min(max(0, self._scroller_pos), super().get_height() - self._scroller_size)
            relative_pos = self._scroller_pos / (super().get_height() - self._scroller_size)
